...
HASHENTRY HashTableElse[MAX_HASHENTRIES];
...
void InitHashTables() {
	...
	memset(HashTableElse,0,sizeof(HASHENTRY)*MAX_HASHENTRIES);
}
...
void GGStoreHashTable(HASHKEY hashkey)
{
	...
	entry->lock = hashkey;
	entry->gen  = IdaInfo->CurrentHashGen;
}
...
int GGGetHashTable(HASHKEY hashkey)
{
	...
	if (entry->lock==hashkey && entry->gen==IdaInfo->CurrentHashGen)
		return(YES);
	else
		return(NO);
}
...
{
  if( entry->gen < IdaInfo->CurrentHashGen )
    return -1;
  if( entry->down > down )
    return 1;
...
if( QualityCompare( entry, down ) > 0 ) {
  ...
  memcpy( entry + 1, entry, sizeof( HASHENTRY ) );
	...
	/* check if a frontier node */
	if (   (entry->backward==1 && entry->g>=IdaInfo->BackDepthLimit)
	    || (entry->backward==0 && entry->g>=IdaInfo->ForwDepthLimit)) {

		/* Rehash if itself frontier node */
		if (   (back==0 && g>=IdaInfo->BackDepthLimit)
		    || (back==1 && g>=IdaInfo->ForwDepthLimit)) {
			SR(Debug(4,g,
			    "rehash because of frontier collision\n"));
			/* Note: XXXX */
		} else {
			SR(Debug(4,g,
			    "Can't store in hashtable, frontier collision\n"));
			return(NULL);
		}
	}
	...
	entry->dlsearched = dl;
	entry->pensearched= pen;
	entry->backward   = back;
	entry->pathflag   = pathflag;
